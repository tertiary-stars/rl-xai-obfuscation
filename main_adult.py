...
from src.utils import load_and_train_adult_xgb

SEED = 42
MONITOR_DIR = "training_logs_adult"
...
if __name__ == "__main__":
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Starting execution for Adult Income pipeline...")
    
    try:
        X_train, X_test, target_model, explainer = load_and_train_adult_xgb()
        
        ablation_pairs = [(0.0, 1.0), (1.0, 0.5), (1.0, 0.2), (1.0, 0.1), (1.0, 0.05), (1.0, 0.01), (1.0, 0.0)]

        for lam, mu in ablation_pairs:
            print(f"\n--- Training adult agent with lambda={lam}, mu={mu} ---")
            ...
            model_name = f"ppo_xai_defender_adult_l{lam}_m{mu}"
